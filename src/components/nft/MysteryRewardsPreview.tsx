  ...
  return (
    <div className={`space-y-2.5 ${className}`}>
      <div className="flex items-center gap-1.5 text-xs font-semibold text-gray-700 dark:text-gray-300">
        <span>🎁</span>
        <span>Phan thuong tiem nang:</span>
      </div>
      ...
      <div className="space-y-1.5 max-h-40 overflow-y-auto custom-scrollbar">
        ...
        {rewards.nfts?.map((nft, index) => (
          <div
            key={`nft-${index}`}
            className="flex items-center justify-between text-xs px-3 py-2 rounded-lg bg-gradient-to-r from-purple-50 to-pink-50 dark:from-purple-900/20 dark:to-pink-900/20 border border-purple-200 dark:border-purple-800"
          >
            <div className="flex items-center gap-2">
              {nft.image ? (
                <img
                  src={nft.image}
                  alt={nft.name}
                  className="w-7 h-7 rounded border-2 border-white dark:border-gray-700 object-cover"
                />
              ) : (
              ...
              <div className="flex flex-col gap-1">
                ...
                <LevelBadge level={nft.rarity} showTooltip={false} />
              </div>
            </div>
            {nft.probability && (
              <div className="flex items-center gap-1 px-2 py-0.5 rounded-full bg-purple-200 dark:bg-purple-800 text-[10px] font-semibold">
                <span>🎯</span>
      ...
      <div className="text-[10px] text-center text-gray-500 dark:text-gray-400">
        {(rewards.tokens?.length || 0) + (rewards.nfts?.length || 0)} loai
        phan thuong
      </div>
    </div>
  );
